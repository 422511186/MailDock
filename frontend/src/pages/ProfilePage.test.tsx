...
describe('ProfilePage', () => {
  ...
  it('渲染只读资料（邮箱）与显示名输入框', () => {
    const api = stubApi();
    render(<ProfilePage api={api as never} user={user()} onUserUpdated={vi.fn()} />);
  ...
  it('提交改名调用 updateDisplayName 并回传更新后的用户', async () => {
    ...
    const api = stubApi();
    render(<ProfilePage api={api as never} user={user()} onUserUpdated={onUserUpdated} />);
  ...
  it('linux.do 用户（hasPassword=false）改密区禁用并提示', () => {
    const api = stubApi();
    render(<ProfilePage api={api as never} user={user({ hasPassword: false })} onUserUpdated={vi.fn()} />);
  ...
  it('页面容器使用与账号列表一致的自适应宽度（app-main）', () => {
    const api = stubApi();
    const { container } = render(
      <ProfilePage api={api as never} user={user()} onUserUpdated={vi.fn()} />
    );
  ...
  it('linux.do 提示使用琥珀色样式（对齐原型，非蓝色）', () => {
    const api = stubApi();
    render(
      <ProfilePage api={api as never} user={user({ hasPassword: false })} onUserUpdated={vi.fn()} />
    );
