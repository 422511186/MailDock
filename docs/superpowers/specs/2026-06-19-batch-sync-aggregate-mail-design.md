...
- 在 `Database.initSchema()` 中新增一张 external-content FTS5 虚拟表，幂等创建（`CREATE VIRTUAL TABLE IF NOT EXISTS`）：
  ...
  - 分词器选 `trigram`：对中文友好，支持任意子串匹配（走索引），适配中文邮件。
  - external content（`content='mail_message'`）：FTS 表不重复存储 `body_text`，节省空间，索引数据从 `mail_message` 取。
- 用触发器保持同步（同样在 `initSchema` 中幂等创建 `CREATE TRIGGER IF NOT EXISTS`）：
  - `AFTER INSERT ON mail_message`：写入 FTS。
  - `AFTER DELETE ON mail_message`：用 `'delete'` 特殊行删除 FTS 项（external content 删除需带 old 值）。
  - `AFTER UPDATE ON mail_message`：先 delete 再 insert。当前邮件入库后基本不可变（仅 `is_read` 变化，不影响索引列），但仍补全 UPDATE 触发器以防未来字段变更导致索引漂移。
- 首次启动回填：existing 库已有邮件但 FTS 为空时，执行 `INSERT INTO mail_message_fts(mail_message_fts) VALUES('rebuild')` 一次性重建。用计数比对（FTS 行数 != 邮件行数）作为守卫，避免每次启动都重建。回填经 `runWrite` 串行化。
...
- 过滤条件（全部参数化，按需拼接）：
  - 关键词（含正文）：
    - 关键词长度 ≥ 3：`JOIN mail_message_fts f ON f.rowid = m.id` 且 `mail_message_fts MATCH ?`，覆盖主题 + 发件人 + 正文，走 FTS 索引。查询串做转义（包裹双引号作为 FTS 短语），防止 FTS 语法字符注入。
    - 关键词长度 < 3：trigram 无法用 MATCH，回退 `(subject LIKE ? OR from_addr LIKE ?)`（不扫正文，避免全表扫描）。
...
- 工具栏：关键词输入框（提示「搜索主题 / 发件人 / 正文」）、账号下拉（全部 / 某账号）、已读三态（全部 / 已读 / 未读）、附件三态（全部 / 有 / 无）、时间范围、排序，并配「收取全部」按钮。
...
- Repository：临时 SQLite 文件库，造多账号、多邮件样本，验证各过滤条件组合、用户隔离（不返回他人邮件）、分页、排序白名单拒绝非法字段。
  - FTS 专项：验证正文关键词命中（中文子串）、短词（<3 字符）回退 LIKE 行为、FTS 语法字符不致报错、新邮件入库后立即可被全文检索（触发器生效）、回填重建后旧邮件可检索。
...
- SQLite 写串行化：本次聚合查询为只读，不加锁；批量收信复用既有写路径（`runWrite`）。FTS 触发器随 `mail_message` 写自动执行，仍在写锁内；首次回填经 `runWrite` 串行化。
- 授权码不外泄：聚合查询只返回邮件摘要，不涉及账号授权码。
- 用户数据隔离：所有新查询强制 `a.user_id = ?`，跨用户邮件按不存在处理。
- SQL 注入防护：过滤值参数化，排序字段 / 方向用白名单；FTS 关键词包裹双引号作为短语，避免 FTS 语法字符被解释。
