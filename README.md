...
当前版本已支持预制邮箱密码用户与 linux.do OAuth 登录。用户资料、邮箱账号、邮件和附件均按当前登录用户隔离。

## 架构概览

请求流向：React 前端（react-router）→ `/api/v1` REST API → Vert.x Router（`ApiRouter`）→ 领域 Handler → Service → Repository → SQLite。

**后端结构**
- `bootstrap/`：应用装配层。`AppWiring` 负责实例化 Repository、Service、Router、静态资源处理器，并返回 `AppRuntime`。
- `web/WebVerticle`：只负责启动 / 停止 HTTP 服务。
- `web/ApiRouter`：拼装 `/api/v1` 路由表、挂载公共中间件和统一 failure handler。
- `web/handler/`：按领域拆分的 HTTP handler（认证、Session、账号、邮件、用户）。
- `web/support/`：handler 共享工具，如统一 JSON 响应、请求体读取、Cookie 处理、路径参数与当前用户提取。
- `service/`：核心业务层；其中账号领域已进一步拆成 `AccountConnectionTester`、`AccountImporter`、`AccountDeletionService`。
- `storage/AttachmentStorage`：附件落盘文件的读写、删除与路径解析。

**前端结构**
- `App.tsx` 使用 `BrowserRouter + Routes + ProtectedRoute` 管理页面路由。
- 账号页主流程在 `pages/AccountsPage.tsx`，纯展示 / 派生逻辑拆到 `pages/accountsPageModel.ts`。
- 账号页弹窗与行菜单拆到 `pages/accounts/`，包括新增账号、批量导入、删除确认、测活确认、三点菜单。
- 通用弹窗外壳在 `components/Modal.tsx`。
...
MAILDOCK_FRONTEND_URL=http://localhost:5173
...
# 可选：OAuth 出站 HTTP 代理
MAILDOCK_HTTP_PROXY_HOST=127.0.0.1
MAILDOCK_HTTP_PROXY_PORT=7890
...
### UI/UX
- ✅ 现代化响应式设计（Tailwind CSS + lucide-react）
- ✅ 品牌顶栏、渐变按钮、白色卡片式数据区
- ✅ Toast 通知提示
- ✅ 长邮箱中间省略并通过悬浮 title 展示完整值
- ✅ 账号状态徽章、彩色头像、三点行操作菜单
- ✅ 页大小选择（10/20/50/100）
...
│   │   ├── bootstrap/         # AppWiring / AppRuntime，负责依赖装配
│   │   ├── config/            # 配置加载
│   │   ├── mail/              # IMAP 客户端与邮件解析
│   │   ├── model/             # 数据模型
│   │   ├── repository/        # 数据访问层
│   │   ├── security/          # 加密与 Session
│   │   ├── service/           # 业务服务与账号子服务
│   │   ├── storage/           # 附件文件存储
│   │   └── web/               # Verticle、Router、handler、support
│   └── src/test/               # 后端测试
├── frontend/                    # 前端（React + TypeScript）
│   ├── src/
│   │   ├── api/               # API 客户端
│   │   ├── components/        # Header、Modal、ProtectedRoute、UserMenu 等复用组件
│   │   ├── contexts/          # 认证与主题上下文
│   │   ├── pages/             # 路由页面
│   │   │   ├── accounts/      # 账号页的子组件与确认弹窗
│   │   │   └── accountsPageModel.ts
│   │   ├── utils/             # 前端工具函数（如邮箱省略）
...
**当前用户**
- `PATCH /api/v1/users/me` - 更新当前用户显示名
- `POST /api/v1/users/me/password` - 修改当前用户密码（仅邮箱密码用户）
...
7. **OAuth 跳转**：生产环境请配置 `MAILDOCK_PUBLIC_BASE_URL` 与 `MAILDOCK_FRONTEND_URL`，否则 linux.do 回调或登录后跳转可能不符合部署地址
8. **线程模型**：Vert.x 事件循环 + Worker 线程池，所有阻塞操作必须在 `executeBlocking` 中执行
